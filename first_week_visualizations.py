...
def _get_first_week_lookback(
    ...
    weekly_starts = generate_trading_weeks(
        all_dates, CONFIG["full_period_start"], CONFIG["full_period_end"]
    )

    if not weekly_starts:
        raise ValueError("No trading weeks found")

    first_week_start = weekly_starts[0]
    prev_dates = all_dates[all_dates < first_week_start]

    if len(prev_dates) < 245:
        raise ValueError("Not enough data for 250-day lookback")
...
def plot_first_week_capm_scatter(all_data: Optional[dict] = None) -> bool:
    ...
    try:
        ...
        for idx, token in enumerate(tokens):
            ...
            ax.scatter(index_ret, token_ret, alpha=0.3, s=10, c="gray")
            ...
            ax.set_title(
                f"{token}: Beta={beta:.4f}, Alpha={alpha:.6f}",
                fontsize=10,
                fontweight="bold",
            )

            if col == 0:
                ax.set_ylabel("Token Return", fontsize=8)
            if row == 4:
                ax.set_xlabel("Market Return", fontsize=8)
...
def plot_first_week_raw_vs_residual(all_data: Optional[dict] = None) -> bool:
    logger.info("\n" + "=" * 80)
    logger.info("GENERATING RAW RETURNS vs CAPM RESIDUALS (PAPER FORMAT)")
    logger.info("=" * 80)
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # Figure layout: 10 rows × 4 columns
        # ------------------------------------------------------------------
        fig, axes = plt.subplots(10, 4, figsize=(16, 18), sharex="col")

        # Column titles (top row only)
        axes[0, 0].set_title("Return Series", fontsize=11, fontweight="bold")
        axes[0, 1].set_title("CAPM Residuals", fontsize=11, fontweight="bold")
        axes[0, 2].set_title("Return Series", fontsize=11, fontweight="bold")
        axes[0, 3].set_title("CAPM Residuals", fontsize=11, fontweight="bold")

        # ------------------------------------------------------------------
        # Plot: 2 tokens per row
        # ------------------------------------------------------------------
        for i in range(0, len(tokens), 2):
            ...
            # ================= TOKEN A =================
            token_a = tokens[i]
            prices_a = lookback_data[token_a]["close"]
            raw_a = prices_a.pct_change().dropna()
            res_a = capm_results[token_a]["residuals"]

            axes[row, 0].plot(
                raw_a.index, raw_a.values, linewidth=0.8, color="steelblue"
            )
            axes[row, 0].set_ylabel(token_a, fontsize=9, fontweight="bold")
            axes[row, 0].axhline(
                0, color="red", linestyle="--", linewidth=0.5, alpha=0.5
            )
            axes[row, 0].grid(True, alpha=0.3)

            axes[row, 1].plot(
                res_a.index, res_a.values, linewidth=0.8, color="darkorange"
            )
            axes[row, 1].axhline(
                0, color="red", linestyle="--", linewidth=0.5, alpha=0.5
            )
            ...
            # ================= TOKEN B =================
            token_b = tokens[i + 1]
            prices_b = lookback_data[token_b]["close"]
            raw_b = prices_b.pct_change().dropna()
            res_b = capm_results[token_b]["residuals"]

            axes[row, 2].plot(
                raw_b.index, raw_b.values, linewidth=0.8, color="steelblue"
            )
            axes[row, 2].set_ylabel(token_b, fontsize=9, fontweight="bold")
            axes[row, 2].axhline(
                0, color="red", linestyle="--", linewidth=0.5, alpha=0.5
            )
            ...
            axes[row, 3].plot(
                res_b.index, res_b.values, linewidth=0.8, color="darkorange"
            )
            axes[row, 3].axhline(
                0, color="red", linestyle="--", linewidth=0.5, alpha=0.5
            )
        ...
        # ------------------------------------------------------------------
        figures_dir = ensure_dir(Path(CONFIG["results_dir"]) / "figures")
        output_path = figures_dir / "first_week_raw_vs_residual_all_tokens.png"
        plt.savefig(output_path, dpi=300, bbox_inches="tight")
        plt.close()
        ...
        logger.error(f"Failed to generate raw vs residual plots: {e}", exc_info=True)
        return False
...
def compute_and_save_delta_matrices(
    ...
    try:
        ...
        plt.title("Delta H Matrix", fontsize=16, fontweight="bold")
        plt.xlabel("Token", fontsize=12)
        plt.ylabel("Token", fontsize=12)
        plt.xticks(rotation=45, ha="right")
        plt.yticks(rotation=0)
        plt.tight_layout()
        delta_H_png = figures_dir / "Table_9_delta_H.png"
        plt.savefig(delta_H_png, dpi=300, bbox_inches="tight")
        plt.close()
        logger.info(f"✅ Saved ΔH matrix (PNG): {delta_H_png}")
        ...
        plt.title("Delta Alpha Matrix", fontsize=16, fontweight="bold")
        plt.xlabel("Token", fontsize=12)
        plt.ylabel("Token", fontsize=12)
        plt.xticks(rotation=45, ha="right")
        plt.yticks(rotation=0)
        plt.tight_layout()
        delta_alpha_png = figures_dir / "Table_10_delta_alpha.png"
        plt.savefig(delta_alpha_png, dpi=300, bbox_inches="tight")
        plt.close()
        logger.info(f"✅ Saved Δα matrix (PNG): {delta_alpha_png}")
